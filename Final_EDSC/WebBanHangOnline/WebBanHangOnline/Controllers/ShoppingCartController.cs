...
namespace WebBanHangOnline.Controllers
{
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    // Lưu thông tin mã giảm giá vào đơn hàng nếu có
                    if (Session["Coupon"] is Coupon coupon)
                    {
                        ...
                        order.TotalAmount = CalculateTotalWithCoupon();
                    }
                    else
                    {
        ...
        {
            var result = new { isValid = false, totalFormatted = "" };

            // Tìm mã giảm giá theo Code
            var coupon = db.Coupons.SingleOrDefault(c => c.Code == code);

            // Kiểm tra mã giảm giá có hiệu lực (ngày hết hạn và trạng thái IsActive)
            if (coupon != null && coupon.IsActive && coupon.ExpirationDate >= DateTime.Now)
            {
                // Lưu thông tin mã giảm giá vào Session
                Session["Coupon"] = coupon;

                // Gọi hàm tính lại tổng tiền với mã giảm giá
                var updatedTotal = CalculateTotalWithCoupon();

                result = new { isValid = true, totalFormatted = updatedTotal.ToString("N0") };
            }

            return Json(result, JsonRequestBehavior.AllowGet);
        }

        private decimal CalculateTotalWithCoupon()
        {
            var cart = (ShoppingCart)Session["Cart"];
            decimal total = cart.Items.Sum(item => item.TotalPrice);

            // Kiểm tra nếu có mã giảm giá trong Session và áp dụng cho tổng tiền
            if (Session["Coupon"] is Coupon coupon)
            {
